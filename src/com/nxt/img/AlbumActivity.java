...
import com.nxt.img.adapter.AlbumGridViewAdapter;
import com.nxt.wly.AbsMainActivity;
import com.nxt.wly.R;

public class AlbumActivity extends AbsMainActivity {
	...
	private void init() {
		...
		chanceBT.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(AlbumActivity.this, AlbumChanceActivity.class);
		...
		iv_back=(ImageView)findViewById(R.id.set_cancel);
		...
		gridImageAdapter.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
			...
			public void onItemClick(final CheckBox toggleButton, int position, final String path, boolean isChecked) {
				if (selectedDataList.size() >= 9) {
					...
					if (!removePath(path)) {
						Toast.makeText(AlbumActivity.this, "最多只能选择9张", Toast.LENGTH_SHORT).show();
					}
					return;
				}
				if (isChecked) {
		...
		okButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (booleanExtra) {
					...
					intent1.putExtra("dataList", selectedDataList);
					setResult(0, intent1);
					finish();*/
					intent = new Intent(AlbumActivity.this, AlbumEditActivity.class);
